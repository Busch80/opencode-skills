...
12. **3-stufige Timeline als bevorzugte Visualisierung**: Für Problem-Lösung-Weg mit 2 Tools (RMM + EDR) eine 3-stufige Timeline verwenden: Ohne System → Tool 1 → Tool 2 [End-State, dunkel]. H2 „Vom Chaos zur Kontrolle" o. ä., 3 Bullets pro Stufe, custom SVG-Icons mittel-detailliert mit Glow/Schatten, einheitliche cyan-Pfeile. KEIN 4. „Unter Kontrolle"-Step (zu viel Versprechen gemäß kpx-schweiz-marketing).
13. **Grosszuegiger Abstand nach Visualisierungs-Sektionen**: Wenn eine Sektion eine zentrale Visualisierung enthaelt (Timeline, Diagramm, Vorher-Nachher, Mini-Flow), braucht die Folgesektion `mt-12 md:mt-16`. Default-`gap-5` zwischen Karten innerhalb der Folgesektion reicht NICHT, um die Visualisierungs-Sektion als eigenstaendige Einheit zu trennen. Visuelle Sektionen brauchen mehr Luft zur naechsten Sektion als gewöhnliche Sections.
14. **Service-spezifische Chevron-Diagramm-Komponenten**: Wenn `ServiceModelArrowsFull` fuer einen neuen Service wiederverwendet werden soll, aber die Inhalte (Plattform, Phasen, Modelle) abweichen, **neue separate Komponente erstellen** statt zu generalisieren. Network verwendet `ServiceModelArrowsFullNetwork` mit UniFi + Pro/Max-Switches und Network-spezifischen Phasen. Code-Duplikation wird bewusst in Kauf genommen fuer klare Trennung der Verantwortlichkeiten. Nur generalisieren, wenn die gleiche Komponente in 3+ Services mit identischer Struktur genutzt wird.