...
| 8 | **Chevron-Diagramm (Drei Betreuungsmodelle)** | light | `kpx-section-light` | `ServiceModelArrowsFull` mit Reihenfolge Rundum / Gemeinsam / Eigenregie. Reihenfolge NICHT ändern — konsistent mit `externe-it-abteilung`. heading/subheading via Props setzen. | `ServiceModelArrowsFull` aus `@/components/ServiceModelArrowsFull`. Daten aus `serviceModelData.ts` (`MODELS_FULL`). Default-Props für `externe-it-abteilung`, page-spezifische Props für andere Seiten. |
